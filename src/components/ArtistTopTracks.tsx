  ...
  return (
    <section className="w-full">
      ...
      <div className="divide-y divide-border">
        {tracks.map((track, i) => {
          const row = (
            <div className="flex items-center gap-3 py-3 px-2 group-hover:bg-border group-hover:rounded transition-colors">
          ...
          return track.albumName ? (
            <Link
              ...
              className="scroll-fade-in group block"
            >
              {row}
            </Link>
          ) : (
            <div key={track.name + i} className="scroll-fade-in hover:bg-border rounded transition-colors">{row}</div>
          );
        })}
      </div>
    </section>
